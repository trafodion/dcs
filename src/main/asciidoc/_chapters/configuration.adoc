...
link:href=http://en.wikipedia.org/wiki/Network_Time_Protocol[NTP] on your cluster, or an equivalent.
...
section that follows, <<configuration,configuration>>. 
...
_DcsServer_s, one host per line or the host name followed by the number of master executor servers. 
...
configuration, see section <<dcs.default.configuration,DCS Default Configuration>>. Look for the `dcs.zookeeper.property` prefix 
...
(It binds to the local machine only and remote clients will not be able to connect). <note xml:id="how_many_zks">
...
the file _conf/dcs-site.xml_. For the list of configurable properties, see <<dcs.default.configuration,DCS Default Configuration>>
