...
// 
		...
		try {
			...
			if(LOG.isDebugEnabled()){
				...
				System.out.println(serverWorkerName + ". maxRowCnt :" + maxRowCnt);
				System.out.println(serverWorkerName + ". maxRowLen :" + maxRowLen);
...
//=====================Process ServerApiSqlFetch===========================
    		...
    		if(LOG.isDebugEnabled())
    			System.out.println(serverWorkerName + ". outParamLength :" + maxRowLen);
            ...
            try {
				while( true){
					...
					int dataType = 0;
					int dataMaxLen = 0;
	                System.out.println(serverWorkerName + ". rs.next()");
					if (false == rs.next()){
						break;
					}
                    System.out.println(serverWorkerName + ". after rs.next()");
					...
					if(LOG.isDebugEnabled()){
						System.out.println(serverWorkerName + ". outNumParam :" + outNumParam);
						System.out.println(serverWorkerName + ". maxRowLen :" + maxRowLen);
						System.out.println(serverWorkerName + ". totalOutLen :" + totalOutLen);
					}
					...
					curRowNumber++;
					if(LOG.isDebugEnabled()){
						System.out.println(serverWorkerName + ". curRowNumber :" + curRowNumber + " maxRowCnt :" + maxRowCnt);
					}
				...
				System.out.println(serverWorkerName + ". Fetch.SQLException " + ex);
...
//      
    		...
    		if (returnCode != ServerConstants.SQL_SUCCESS && returnCode != ServerConstants.SQL_NO_DATA_FOUND) {
                dataLength += Constants.INT_FIELD_SIZE; 				//totalErrorLength
                if (errorList != null)
            ...
            dataLength += Constants.INT_FIELD_SIZE; 				//outValuesFormat
...
//=========================Update header================================ 
            ...
            System.out.println(serverWorkerName + ". Fetch.UnsupportedEncodingException :" + ue);
            clientData.setRequestAndDisconnect();
        } catch (Exception e){
            System.out.println(serverWorkerName + ". Fetch.Exception :" + e);
            clientData.setRequestAndDisconnect();
        }
        reset();
    ...
    byte[] formatSqlT4Output(Descriptor2 dsc, byte[] sqlarray, long curOutPos, byte[] outValues, ByteOrder bo) throws UnsupportedEncodingException{
		...
		case ServerConstants.SQLTYPECODE_CHAR:
            System.out.println("------------------------formatSqlT4Output: SQLTYPECODE_CHAR :" + Arrays.toString(sqlarray));
			break;
        ...
        case ServerConstants.SQLTYPECODE_VARCHAR_WITH_LENGTH:       //-601
        	System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_VARCHAR :" + Arrays.toString(sqlarray));
 		    break;
		case ServerConstants.SQLTYPECODE_DATETIME:
            System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_DATETIME :" + Arrays.toString(sqlarray));
			switch(datetimeCode){
		...
		case ServerConstants.SQLTYPECODE_INTERVAL:
            System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_INTERVAL :" + Arrays.toString(sqlarray));
            len = tb.getShort();
			dst = new byte[len];
			System.arraycopy(sqlarray, 2, dst, 0, len);
            ...
            System.out.println("formatSqlT4Output: sqlarray :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_INTEGER:					//4
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_INTEGER :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_INTEGER_UNSIGNED:
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_INTEGER_UNSIGNED :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_SMALLINT:
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_SMALLINT :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_SMALLINT_UNSIGNED:
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_SMALLINT_UNSIGNED :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_LARGEINT:
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_LARGEINT :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_DECIMAL:
		case ServerConstants.SQLTYPECODE_DECIMAL_UNSIGNED:
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_DECIMAL :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_IEEE_REAL:                    //6
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_IEEE_REAL :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_IEEE_FLOAT:                   //7
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_IEEE_FLOAT :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_IEEE_DOUBLE:                  //8
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_IEEE_DOUBLE :" + Arrays.toString(sqlarray));
			break;
		case ServerConstants.SQLTYPECODE_NUMERIC:
		case ServerConstants.SQLTYPECODE_NUMERIC_UNSIGNED:
        System.out.println("--------------------------formatSqlT4Output: SQLTYPECODE_NUMERIC :" + Arrays.toString(sqlarray));
            switch (len) {
            case 2:
		...
		default:
            System.out.println("--------------------------formatSqlT4Output: default :" + Arrays.toString(sqlarray));
			break;
		}
