...
public class FloatingIp {
    ...
    public synchronized int runScript() throws Exception {
        ...
        if(externalInterface.equalsIgnoreCase("default")) {
            LOG.info("When floating IP feature is enabled the property " + Constants.DCS_MASTER_FLOATING_IP_EXTERNAL_INTERFACE + " must be specified in your dcs-site.xml");
            return 0;
        }
        ...
        if(externalIpAddress.equalsIgnoreCase("default")) {
            LOG.info("When floating IP feature is enabled the property " + Constants.DCS_MASTER_FLOATING_IP_EXTERNAL_IP_ADDRESS + " must be specified in your dcs-site.xml");
            return 0;
        }
